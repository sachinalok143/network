...
int sendBufferPos;
int sendBackIdx;
int sendBufferBackIdx; 
int sendBufferAvlSpace;
int recvBufferPos;
int recvBackIdx;
int recvBufferBackIdx;
int recvBufferFilledSpace;
char recvBuffer[recvBufferSize];
char sendBuffer[sendBufferSize] ;
int cwnd,rwnd=1;
int ssth, seqToSend;
int pSend[2];
int  max_ack_recvd, max_ack_sent, dup_ack_count,sendcnt,rcvcnt;
float fraction_wnd;
int pRecv[2];
...
int appSend(char * buf, int size, int *pSend)
{
	...
	return write(pSend[1],buf,size);
}

int appRecv(char * buf, int size , int *pRecv)
{
	int n= read(pRecv[0],buf,size);
	return n;
...
void *sendBufferHandler(void* V)
{
    ...
    for(;;)
    {
    	...
    	if(!(n<=sendBufferAvlSpace))
    		n=sendBufferAvlSpace;
...
void *parse_packets(void* V)
{
	...
	for(;;)
	{
		...
		if(i==DATA)
		{
			...
			{
				if(!((recvBufferSize-recvBufferBackIdx)<size))
					memcpy(recvBuffer+recvBufferBackIdx,buf+HEAD,size);
				else
				{
...
int main(int argc, char **argv) {
	...
	if(fork()!=0)
	{
		...
		if(argc==3)
		{
            ...
            {
            	i=i+appSend(buf,n, pSend);
                //cout<<i<<" sent\n";

            }

            cout<<"File/Data sent from APPLICATION LAYER\n";
        }
        else if(argc==2)
        {
